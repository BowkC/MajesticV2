...
import { applyEmbedStructure, errorEmbed } from '../../handlers/helperFunctions';
...
module.exports = {
    ...
    execute: async (client: CustomClient, args: string[], interaction: Message | CommandInteraction, prefix: string) => {
        try {
            ...
            async function infoEmbed(cpuUsage: string) {
                const embed = new EmbedBuilder()
                    ...
                    .addFields({ name: 'Prefix', value: `\`${prefix}\`` },
                        ...
                        { name: 'API Ping', value: `\`${Math.round(client.ws.ping)}ms\``, inline: true }, { name: 'Client Latency', value: `\`${latency}ms\``, inline: true },
